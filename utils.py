...
"""
@filename: utils.py
@description: functions related to prompt construction, agent configuration, modified from Qihao
@time: 2025/06/22
@author: Haoli WANG
@Version: 1.0
"""
...
def set_student_agent(
    client, student, topic, history, start_i, welcome_words, cut_word_length, usercollection
):
    """
    Description: 
        set student agent
    Parameters:
        client(Obj): client created by OpenAI mode
        student(dict): student info (id and name)
        topic(str): the dicussion topic of this room
        history(list): the conversation history of this dicussion room
        start_i(int): Determine the position from which the agent should start reviewing historical conversations
        welcome_words(str): words used at the beginning of discussion
        cut_word_length(int): max length of the prompt
        usercollection(mondodb collection): the user collection, use to get more student info
    Returns:
    ...
    Outputs:
        output1(type): desc
    """
    memberInfo = usercollection.find_one({"_id": student["memberId"]})
    client = OpenAI(
        base_url="https://aihubmix.com/v1",
        api_key="sk-4CFE7AOiCwVbiRXw65A86b969f204a98B4Dd5bB0D97c9b70",
    )
...
def select_student(members, history, except_members=[]):
    """
    Description: 
        random select a student from a room
    Parameters:
        members(list): 
        param1(type): desc
        param1(type): desc
    ...
    if len(history) < 1:
        # conversation start
        specific_set = [m for m in members if m["memberId"] not in except_members]
    else:
        # Filter out the member who spoke last
        lateset_conversation = history[-1]  # get the latest conversations
        specific_set = [
            m
            for m in members
            if m["memberId"] != lateset_conversation["role_id"]
            and m["memberId"] not in except_members
        ]
    return random.choice(specific_set)
...
def student_calling(student_agent, mode="student", time=0):
        ...
        conversation_now.append(
            {
                f"message-{ len(history) + 1}": {
                    "speaker": selected_student["memberName"],
